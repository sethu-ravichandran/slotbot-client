...
const Schedule = () => {
  ...
  useEffect(() => {
    ...
    const fetchSchedule = async () => {
      try {
        const response = await api.get(`nylas/events`)
  ...
  const handleConnectCalendar = async () => {
    ...
    try {
      const response = await api.post(`nylas/connect`, {})
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="mt-2 sm:mt-0">
          {!isConnected ? (
            ...
            <Button variant="outline">Calendar connected!</Button>
          )}
        </div>
      </div>
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <div className="max-h-[480px] overflow-y-auto border-b border-gray-200">
          <div className="grid grid-cols-7 min-h-[480px]">
            {weekDays.map((day) => {
              const dayMeetings = scheduledMeetings.filter(
                (meeting) =>
                  format(meeting.day, 'yyyy-MM-dd') ===
                ...
                >
                  {dayMeetings.map((meeting) => (
                    <div
                      ...
                      style={{
                        top: `${
                          (parseInt(meeting.startTime.split(':')[0]) - 9) * 20
                        }%`,
                        height: `${
                          (parseInt(meeting.endTime.split(':')[0]) -
                            parseInt(meeting.startTime.split(':')[0])) *
                          20
                        }%`
                      }}
                    >
