...
const CandidateAvailability = () => {
  ...
  const [endTime, setEndTime] = useState(
    addHours(setHours(setMinutes(new Date(), 0), 9), 1)
  )
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      if (slotToDelete._id) {
        await api.delete(`availability/${slotToDelete._id}`)
        ...
        <div className="px-4 py-5 sm:p-6">
          ...
          <div className="mt-5">
            <Button
              ...
              icon={<Plus size={18} />}
            >
              Add Time Slot ({existingTimeSlots.length + newTimeSlots.length}/
              {MAX_SLOTS})
            </Button>
          </div>
        </div>
            ...
            {allSlots.map((slot) => (
              <li key={slot._id || slot.id} className="px-4 py-4 sm:px-6">
                ...
                <div className="mt-2 ml-7 text-sm text-gray-500">
                  <Clock
                    className="mr-2 inline-block h-4 w-4 text-gray-400"
                    aria-hidden="true"
                  />
